...
function registerAppControllers() {
  ...
  app.controller("MainController", [
    ...
    "$rootScope",
    "$timeout",
    function ($scope, $localStorageService, $crmapi, $rootScope, $timeout) {
          ...
          $crmapi.fetchRecordDetailsFromEmail($scope.email, $scope.module).then(function (e, res) {
            ...
            if ($scope.view !== "login") {
              ...
              $scope.currentUserFirstName = $localStorageService.get("first_name")
                ? $localStorageService.get("first_name")
                : $scope.currentUserLastName;
              ...
              var preferenceModuleUrl =
                $scope.userCrmInstanceUrl +
                "/index.php?module=Users&view=PreferenceDetail&parent=Settings&record=" +
                crmUserId;
  ...
  app.controller("LoginController", [
    ...
    "$crmapi",
    "$timeout",
    function ($scope, $localStorageService, $crmapi, $timeout) {
        ...
        $crmapi.resolveURL($scope.username, $scope.password).then(function (e, res) {
          ...
          if (e) {
            ...
            if (res.hasOwnProperty("account") && res.hasOwnProperty("user")) {
              ...
              if (
                allowedStatuses.indexOf(res.account.status) !== -1 &&
                allowedBillingStatuses.indexOf(res.account.billingstatus) !== -1
              ) {
      ...
      $scope.login = function (valid) {
        ...
        if (valid) {
          ...
          if ($scope.form.username.$invalid) $scope.form.username.$dirty = true;
          else $scope.form.password.$dirty = true;
        }
      };
  ...
  app.controller("CreateController", [
    ...
    "$location",
    "$route",
    function ($scope, $crmapi, $location, $route) {
  ...
  app.controller("EditController", [
    ...
    "$filter",
    "$timeout",
    function ($scope, $crmapi, $filter, $timeout) {
      ///////////////////////
      $scope.$watch("view", function () {
        if ($scope.view === "edit") {
          var mode = "create";
          ...
          var getFieldValue = function (fieldtype, fieldValue) {
            switch (fieldtype) {
              ...
              case "multicurrency":
                ...
                break;
              case "date":
                fieldValue = $filter("date_format")(fieldValue, $scope.userpreferences.date_format);
                break;
              case "time":
                fieldValue = $filter("time_format")(fieldValue, $scope.userpreferences.hour_format);
                break;
            }
            return fieldValue;
          };
          ...
          var _fetchFormFields = function (record) {
            if ($scope.module) {
              ...
              if ($scope.module == "Potentials" && mode == "create") var fetchSecondaryModuleName = "Contacts";
              else var fetchSecondaryModuleName = false;

              $crmapi
                .describe($scope.module, fetchOutlookConfigurableFields, fetchSecondaryModuleName)
                .then(function (e, res) {
                  $scope.editViewBusy = false;
                  if (e && e.code === 1501) {
                    $scope.$emit("MainController:changeView", { view: "login" });
                    ...
                    angular.forEach(
                      ...
                      function (value) {
                        if (value.editable) {
                          ...
                          if (
                            (toShowThisField ||
                              (value.type.name == "multicurrency" && value.mandatory) ||
                              fixedFields.indexOf(value.name) !== -1) &&
                            skipFields.indexOf(value.name) === -1
                          ) {
          ...
          if (mode === "edit") {
            ...
            $crmapi.fetchRecord($scope.crmRecordId).then(function (e, res) {
              ...
              if (e && e.code === 1501) {
                ...
                var msg = e.message;
                if (e.code === "ACCESS_DENIED") msg = "Access denied";
          ...
          $scope.create = function (valid) {
            if (valid) {
              ...
              if (
                typeof $scope.secondaryModuleFields != "undefined" &&
                mode === "create" &&
                $scope.module == "Potentials"
              ) {
              ...
              $crmapi.saveRecord($scope.module, formValues, recordId, saveJointRecord).then(function (e, res) {
                ...
                if (!e) {
                  ...
                  if ($scope.module == "Accounts") var name = label;
                  else var name = firstname + " " + lastname;
          ...
          $scope.moduleLabel = function () {
            var module = $scope.module.replace(/s$/, "");
            if (module === "Potential") return "Opportunity";
            return module;
          };
        }
      });
   ...
   */
  app.controller("RecordSummaryController", [
    "$scope",
    "$crmapi",
    "$rootScope",
    function ($scope, $crmapi, $rootScope) {
      $scope.modulesStatus = [];
  ...
  app.controller("TimelineController", [
    ...
    "$crmapi",
    "$localStorageService",
    function ($scope, $crmapi, $localStorageService) {
          ...
          $crmapi.history($scope.module, $scope.crmRecordId).then(function (e, res) {
            ...
            if (e && e.code === 1501) {
              ...
              var getTimelineLabel = function (historyEntry) {
                ...
                switch (historyEntry.values.record.module) {
                  ...
                  case "HelpDesk":
                    ...
                    break;
                  case "Potentials":
                    label += "created an opportunity";
                    break;
                  case "Documents":
                    label += "added a document";
                    break;
                  case "Vendors":
                    label += "added vendor";
                    break;
                  ...
                  case "ServiceContracts":
                    label += "added service contract";
                    break;
                  case "Products":
                    label += "added product";
                    break;
                  case "Emails":
                    label += "made an email interaction";
                    break;
                  default:
                    label += " added " + historyEntry.values.record.module + " record ";
                }
                return label;
              };
              ...
              var getDetailViewUrl = function (instanceUrl, module, record) {
                ...
                if (module === "Inbox") {
                  detailViewUrl =
                    instanceUrl + "/index.php?module=Inbox&view=List&mode=getThreadEmails&threadId=" + record;
                }
              ...
              angular.forEach(
                ...
                function (value) {
                  if (value.status === modTrackerLinkStatusCode) {
                    ...
                    feedEntry.detailViewUrl = getDetailViewUrl(
                      crmUrl,
                      feedEntry.values.record.module,
                      feedEntry.values.record.id
                    );
                    this.push(feedEntry);
                  }
                },
          ...
          $scope.getIconClass = function (module) {
            ...
            switch (module) {
              ...
              case "ModComments":
                ...
                break;
              case "HelpDesk":
                iconClass = "vicon-helpdesk";
                break;
              case "Potentials":
                iconClass = "vicon-potentials";
                break;
              case "Emails":
                iconClass = "vicon-emails";
                break;
              case "Cases":
                iconClass = "vicon-cases";
                break;
              case "Documents":
                iconClass = "vicon-documents";
                break;
              case "Vendors":
                iconClass = "vicon-vendors";
                break;
              case "Assets":
                iconClass = "vicon-assets";
                break;
              case "Project":
                iconClass = "fa fa-briefcase";
                break;
              case "Services":
                iconClass = "vicon-services";
                break;
              case "ServiceContracts":
                iconClass = "vicon-servicecontracts";
                break;
              case "Products":
                iconClass = "vicon-products";
                break;
            }
            return iconClass;
          };
  ...
  app.controller("ActivitiesController", [
    ...
    "$filter",
    "$localStorageService",
    function ($scope, $crmapi, $timeout, $filter, $localStorageService) {
      $scope.$watch("tab", function () {
        if ($scope.tab === "activities") {
              ...
              angular.forEach(
                ...
                function (value) {
                  if (value.editable) {
                    ...
                    if (outlookConfigurableFields.length) {
                      if (outlookConfigurableFields.indexOf(value.name) !== -1) toShowThisField = true;
                    ...
                    if (fixedFields.indexOf(value.name) !== -1) {
                      ...
                      switch (value.name) {
                        ...
                        case "eventstatus":
                          if (
                            $scope.userpreferences.defaulteventstatus !== "undefined" &&
                        ...
                        case "activitytype":
                          if (
                            $scope.userpreferences.defaultactivitytype !== "undefined" &&
                      ...
                      if (
                        (toShowThisField || (value.type.name == "multicurrency" && value.mandatory)) &&
                        skipFields.indexOf(value.name) === -1
                      ) {
                        this.push(value);
                      }
          ...
          $scope.create = function (valid) {
            if (valid && $scope.endDateTimeIsGreater && !$scope.followupValidationError) {
              var start_date_time = moment(
                $scope.date_start.fieldValue + " " + $scope.time_start.fieldValue,
                "YYYY-MM-DD HH:mm"
              );
              var end_date_time = moment(
                $scope.due_date.fieldValue + " " + $scope.time_end.fieldValue,
                "YYYY-MM-DD HH:mm"
              );
              ...
              if (formValues["eventstatus"] === "Held" && $scope.followup === "on") {
                if ($scope.followup_date_start && $scope.followup_time_start) {
                  var followup_date_time = moment(
                    $scope.followup_date_start + " " + $scope.followup_time_start,
                    "YYYY-MM-DD HH:mm"
                  );
              ...
              if ($scope.mode === "edit" && $scope.recordId) recordId = $scope.recordId;
          ...
          $scope.showCreateForm = function () {
            $timeout(function () {
              angular
                .element('input[name="date_start"]')
                .datepick(
                  "setDate",
                  $filter("date_format")(moment().format("YYYY-MM-DD"), $scope.userpreferences.date_format)
                );
            });
              ...
              if (!e && res) {
                ...
                $timeout(function () {
                  angular
                    ...
                    .datepick(
                      "setDate",
                      $filter("date_format")(start_date_time.format("YYYY-MM-DD"), $scope.userpreferences.date_format)
                    );
                  angular
                    .element('input[name="due_date"]')
                    .datepick(
                      "setDate",
                      $filter("date_format")(end_date_time.format("YYYY-MM-DD"), $scope.userpreferences.date_format)
                    );
                  angular
                    .element('input[name="time_start"]')
                    .timepicker(
                      "setTime",
                      $filter("time_format")(start_date_time.format("HH:mm"), $scope.userpreferences.hour_format)
                    )
                    .trigger("change");
                  angular
                    .element('input[name="time_end"]')
                    .timepicker(
                      "setTime",
                      $filter("time_format")(end_date_time.format("HH:mm"), $scope.userpreferences.hour_format)
                    )
                    .trigger("change");
                });
          ...
          $scope.$watchGroup(
            ["date_start.fieldValue", "time_start.fieldValue", "due_date.fieldValue", "time_end.fieldValue"],
            function () {
              if (
                $scope.hasOwnProperty("date_start") &&
                $scope.hasOwnProperty("time_start") &&
                $scope.hasOwnProperty("due_date") &&
                $scope.hasOwnProperty("time_end")
              ) {
  ...
  app.controller("EmailsController", [
    "$scope",
    "$crmapi",
    function ($scope, $crmapi) {
      $scope.$watch("tab", function () {
        if ($scope.tab === "emails") {
          var reqFields = ["subject", "description", "assigned_user_id", "from_email"];
          $scope.loading = true;
          $crmapi
            .fetchRelatedRecords($scope.crmRecordId, $scope.relatedModule, { fields: reqFields })
            .then(function (e, res) {
              $scope.loading = false;
              if (e && e.code === 1501) {
                $scope.$emit("MainController:changeView", { view: "login" });
   ...
   */
  app.controller("OpportunitiesListViewController", [
    "$scope",
    "$crmapi",
    "$timeout",
    "$filter",
    function ($scope, $crmapi, $timeout, $filter) {
      $scope.$watch("subview", function () {
        if ($scope.tab === "opportunities" && $scope.subview === "list") {
          ...
          var clearFormValues = function () {
            ...
            $scope.potentialname.fieldValue = getFieldValue(
              $scope.potentialname.type.name,
              $scope.potentialname["default"]
            );
            if ($scope.hasOwnProperty("closingdate")) {
              $scope.closingdate.fieldValue = getFieldValue(
                $scope.closingdate.type.name,
                $scope.closingdate["default"]
              );
            }
            $scope.assigned_user_id.fieldValue = $scope.currentUserId;
            if ($scope.hasOwnProperty("sales_stage")) {
              ...
              if ($scope.sales_stage["default"]) {
                $scope.sales_stage.fieldValue = getFieldValue(
                  $scope.sales_stage.type.name,
                  $scope.sales_stage["default"]
                );
              }
            }
            ...
            if ($scope.hasOwnProperty("opportunity_type")) {
              ...
              if ($scope.opportunity_type["default"]) {
                $scope.opportunity_type.fieldValue = getFieldValue(
                  $scope.opportunity_type.type.name,
                  $scope.opportunity_type["default"]
                );
              }
            }
          ...
          $scope.create = function (valid) {
            if (valid) {
              ...
              if ($scope.module == "Accounts") relatedFieldName = "related_to";
          ...
          $scope.editRecord = function (record) {
            $crmapi.fetchRecord(record.id).then(function (e, res) {
              if (!e && res) {
                ...
                $scope.assigned_user_id.fieldValue = recordDetails["assigned_user_id"].value;
                if ($scope.hasOwnProperty("closingdate")) {
                  $scope.closingdate.fieldValue = $filter("date_format")(
                    recordDetails["closingdate"],
                    $scope.userpreferences.date_format
                  );
                }
                if ($scope.hasOwnProperty("sales_stage")) {
   ...
   * postComment -
   */
  app.controller("OpportunitiesDetailViewController", [
    "$scope",
    "$crmapi",
    function ($scope, $crmapi) {
      $scope.$watch("subview", function () {
          ...
          $scope.postComment = function () {
            ...
            $crmapi.addComment($scope.record.id, commentcontent).then(function (e, res) {
              if (e && e.code === 1501) {
                ...
                if (e.code === "ACCESS_DENIED") message = "You are restricted from adding a comment!";
                vtigerHelper.showDialog(
                  {
                    title: "Error",
                    content: message,
  ...
  app.controller("CasesListViewController", [
    "$scope",
    "$crmapi",
    "$filter",
    "$timeout",
    "$outlookconnector",
    function ($scope, $crmapi, $filter, $timeout, $outlookconnector) {
      $scope.$watch("subview", function () {
        if ($scope.tab === "cases" && $scope.subview === "list") {
          ...
          $scope.showCreateForm = function () {
            ...
            if ($scope.showForm) {
              ...
              angular.forEach($scope.formFields, function (field, index) {
                if (
                  field.name === "group_id" &&
                  $scope.userpreferences.hasOwnProperty("user_primary_group") &&
                  $scope.userpreferences["user_primary_group"]
                ) {
              ...
              function (value) {
                if (value.editable) {
                  ...
                  if (fixedFields.indexOf(value.name) !== -1) {
                    ...
                    if (
                      (value.quickCreate || (value.type.name == "multicurrency" && value.mandatory)) &&
                      value.name !== "contact_id"
                    ) {
                      this.push(value);
                    }
                  }
          ...
          var clearFormValues = function () {
            ...
            if ($scope.casepriority["default"])
              $scope.casepriority.fieldValue = getFieldValue(
                $scope.casepriority.type.name,
                $scope.casepriority["default"]
              );
            if ($scope.hasOwnProperty("description")) {
              $scope.description.fieldValue = getFieldValue(
                $scope.description.type.name,
          ...
          $scope.handleCasePicklistTransitions = function (currentStatusValue) {
            ...
            if (transitionRules !== false) {
              ...
              angular.forEach(
                $scope.casestatuspicklistvalues,
   ...
   * newcomment - new comment
   */
  app.controller("CasesDetailViewController", [
    "$scope",
    "$crmapi",
    function ($scope, $crmapi) {
      $scope.$watch("subview", function () {
   ...
   */
  app.controller("HelpdeskListViewController", [
    "$scope",
    "$crmapi",
    "$filter",
    "$timeout",
      ...
      $scope.$watch("subview", function () {
        if ($scope.tab === "helpdesk" && $scope.subview === "list") {
          ...
          var clearFormValues = function () {
            ...
            $scope.ticket_title.fieldValue = getFieldValue(
              $scope.ticket_title.type.name,
              $scope.ticket_title["default"]
            );
            ...
            if ($scope.ticketstatus["default"])
              $scope.ticketstatus.fieldValue = getFieldValue(
                $scope.ticketstatus.type.name,
                $scope.ticketstatus["default"]
              );
            $scope.ticketpriorities.fieldValue = $scope.ticketpriorities.type.picklistValues[0].value;
            if ($scope.ticketpriorities["default"])
              $scope.ticketpriorities.fieldValue = getFieldValue(
                $scope.ticketpriorities.type.name,
                $scope.ticketpriorities["default"]
              );
            if ($scope.hasOwnProperty("description")) {
              $scope.description.fieldValue = getFieldValue(
                $scope.description.type.name,
   ...
   * newcomment - new comment
   */
  app.controller("HelpdeskDetailViewController", [
    "$scope",
    "$crmapi",
    function ($scope, $crmapi) {
      $scope.$watch("subview", function () {
      ...
      $scope.$watch("tab", function () {
        if ($scope.tab === "comments") {
          ...
          var loadRelatedComments = function () {
            var reqFields = ["commentcontent", "assigned_user_id", "modifiedtime"];
            $crmapi
              .fetchRelatedRecords($scope.crmRecordId, "ModComments", { fields: reqFields })
              .then(function (e, res) {
                if (e && e.code === 1501) {
                  $scope.$emit("MainController:changeView", { view: "login" });
                } else if (res) {
    ...
    function ($scope, $crmapi, $timeout) {
      $scope.$watch("tab", function () {
        if ($scope.tab === "settings") {
          $crmapi.getExtensionModuleSettings().then(function (e, res) {
            if (!e && res) $scope.moduleSettings = res;
            else if (e && e.code === 1501) $scope.$emit("MainController:changeView", { view: "login" });
          });
          ...
          $scope.getIconClass = function (modulename) {
            ...
            switch (modulename) {
              case "Events":
                ...
                break;
              case "Emails":
                iconClassName = "vicon-emails";
                break;
              case "Quotes":
                iconClassName = "vicon-quotes";
                break;
              case "Invoice":
                iconClassName = "vicon-invoice";
                break;
              case "Potentials":
                iconClassName = "vicon-potentials";
                break;
              case "Cases":
                iconClassName = "vicon-cases";
                break;
              case "HelpDesk":
                iconClassName = "vicon-helpdesk";
                break;
              case "ModComments":
  ...
  app.controller("EditIconClickedController", [
    "$scope",
    "$location",
    "$route",
   ...
   *
   */
  app.controller("AttachEmailController", [
    "$scope",
    "$crmapi",
    function ($scope, $crmapi) {
                  ...
                  vtigerHelper.showDialog(
                    {
                      title: "Note",
                      content:
                        "Emails moduled disabled in CRM. \n\
   ...
   */
  app.controller("DetailViewController", [
    "$scope",
    "$crmapi",
    "$filter",
    function ($scope, $crmapi, $filter) {
      $scope.$watch("tab", function () {
        if ($scope.tab === "detail") {
              ...
              $crmapi.fetchRecord($scope.crmRecordId).then(function (e, res) {
                ...
                if (e && e.code === 1501) {
                  ...
                  var getDisplayValue = function (fieldValue, type) {
                    switch (type) {
                      ...
                      case "multipicklist":
                        ...
                        break;
                      case "url":
                        fieldValue = $filter("linky")(fieldValue);
                        break;
                      case "date":
                        fieldValue = $filter("date_format")(fieldValue, $scope.userpreferences.date_format);
                        break;
                  ...
                  angular.forEach(fields, function (value) {
                    ...
                    if (outlookConfigurableFields.length) {
                      if (outlookConfigurableFields.indexOf(fieldName) !== -1) toShowThisField = true;
  ...
  app.controller("ngRouteController", [
    "$route",
    "$rootScope",
    "$timeout",
    "$localStorageService",
    function ($route, $rootScope, $timeout, $localStorageService) {
      $timeout(function () {
