...
function registerAppControllers() {
     ...
     */
    app.controller('MainController', ["$scope","$localStorageService","$crmapi", "$rootScope",function($scope,$localStorageService,$crmapi, $rootScope) {
        ...
        var autoRoute = function() {
            ...
            $scope.$watch('email',function() {
                $crmapi.fetchRecordDetailsFromEmail($scope.email).then(function(e,res) {
                    if(e && e.code === 1501) {
                        clearAppStorage();
                        $scope.view = 'login';
                    } else if(res) {
                        ...
                        $scope.module = res.record.module;
                        $scope.crmRecord = res.record;
                        $scope.name = $scope.crmRecord.firstname + ' ' + $scope.crmRecord.lastname;
            ...
            if($scope.view === 'edit') {
                    ...
                    var _fetchFormFields = function(record) {
                        if($scope.module) {
                            ...
                            $crmapi.describe($scope.module).then(function(e,res) {
                                $scope.editViewBusy = false;
                                if(e && e.code === 1501) {
                                    $scope.$emit("MainController:changeView",{view:'login'});
                                } else if(res) {
                                    ...
                                    angular.forEach(fields,function(value) {
                                        if(value.editable){
                                            ...
                                            if((value.quickcreate || (value.type.name == 'multicurrency' && value.mandatory) || fixedFields.indexOf(value.name) !== -1) && skipFields.indexOf(value.name) === -1) {
                    ...
                    $scope.create = function(valid) {
                        if(valid) {
                            ...
                            $crmapi.saveRecord($scope.module,formValues,recordId).then(function(e,res) {
                        ...
                        angular.forEach(fields,function(value) {
                            if(value.editable){
                                ...
                                if(fixedFields.indexOf(value.name) !== -1) {
                                    ...
                                    if((value.quickcreate || (value.type.name == 'multicurrency' && value.mandatory)) && skipFields.indexOf(value.name) === -1) {
                                        this.push(value);
                                    }
                                }
                            }
                                ...
                                if(e && e.code === 1501) {
                                   ...
                                   var skipFields = ['contact_no','support_end_date','support_start_date','source',
                                    ...
                                    angular.forEach(fields,function(value) {
                                        ...
                                        if(skipFields.indexOf(fieldName) === -1 && value.summaryView){ 
                                            var recordDetails = {
