...
function registerAppServices() {
    ...
    angular.module("VtigerOutlookAddon").factory('$crmapi', ["$deferred","$crmconnector","$localStorageService","$http",function($deferred,$crmconnector,$localStorageService,$http) {
        ...
        return {
            ...
            describe : function(module,fetchOutlookConfigurableFields,fetchSecondaryModuleName) {
                
				
				if (!fetchOutlookConfigurableFields) fetchOutlookConfigurableFields = false;
				
				if (!fetchSecondaryModuleName) fetchSecondaryModuleName = false;
				...
				var params = {
                  ...
                  'fetchSecondaryModuleName' : fetchSecondaryModuleName
            ...
            saveRecord : function(module,values,recordId,saveJointRecord) {
				
				if (!saveJointRecord) saveJointRecord = false;
                ...
                if(!saveJointRecord)
                    var operation = 'saveRecord'; 
                else 
                    var operation = 'saveJointRecord';   
            ...
            fetchRecordDetailsFromEmail : function(email,module) {
			...
			fetchRecordsBySearchKeyword : function(referenceModule,searchKey) {
                var params = {
                    '_operation' : 'fetchRecordsBySearchKeyword',
                    '_session' : $localStorageService.get('vtigersession'),
                    'referenceModule' : referenceModule,
                    'searchKey' : searchKey
                };
